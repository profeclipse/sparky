...
namespace sparky {
	...
	void Shader::setUniformVec2(const GLchar* name,const vec2& value) {
		CHECK_GL(glUniform2f(getUniformLocation(name),value.x,value.y));
	}

	void Shader::setUniformVec3(const GLchar* name,const vec3& value) {
		CHECK_GL(glUniform3f(getUniformLocation(name),value.x,value.y,value.z));
	}

	void Shader::setUniformVec4(const GLchar* name,const vec4& value) {
	...
	GLuint Shader::load() {
		...
		std::string vstext = read_file(m_vertPath);
		std::string fstext = read_file(m_fragPath);
