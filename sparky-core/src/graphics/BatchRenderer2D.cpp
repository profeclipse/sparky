...
namespace sparky {
	namespace graphics {
		...
		void BatchRenderer2D::drawString(const std::string& text,const math::vec3& position,
			...
			for (int i=0; i<text.length(); ++i)
			{
				...
				if (glyph != NULL)
				{
					...
					m_buffer->position = *m_transformBack * math::vec3(x0,y0,0);
					m_buffer->uv = math::vec2(u0,v0);
					m_buffer->tid = ts;
					m_buffer->color = color;
					m_buffer++;

					m_buffer->position = *m_transformBack * math::vec3(x0,y1,0);
					m_buffer->uv = math::vec2(u0,v1);
					m_buffer->tid = ts;
					m_buffer->color = color;
					m_buffer++;

					m_buffer->position = *m_transformBack * math::vec3(x1,y1,0);
					m_buffer->uv = math::vec2(u1,v1);
					m_buffer->tid = ts;
					m_buffer->color = color;
					m_buffer++;

					m_buffer->position = *m_transformBack * math::vec3(x1,y0,0);
